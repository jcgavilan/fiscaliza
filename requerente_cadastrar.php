...
mysqli_report(MYSQLI_REPORT_OFF); // evita exception automática
...
if (!$stmt) {
  fail('Erro interno ao preparar o cadastro.', 'Prepare failed: ' . mysqli_error($conexao));
}

mysqli_stmt_bind_param($stmt, 'sssssssss', $fullName, $cpf,$birthDate,$address,$phones,$email,$password,$flag,$notes);
...
//         echo json_encode([
//                 'ok' => false,
//                 'message' => 'Erro interno ao preparar a operação de cadastro.'
//         ]);
//         exit;
...
if (!mysqli_stmt_execute($stmt)) {
  fail('Erro ao salvar o cadastro.', 'Execute failed: ' . mysqli_stmt_error($stmt));
}
