...
$taxa_valor  = $f->limpa_variavel($_POST['taxa_valor'], 1000, $purifier);
...
// FAZ O CADASTRO DO PEDIDO, PARA GERAR O ID DO PEDIDO E PREENCHER A TABELA DE ARQUIVOS
    ...
    include "classes/gera.hash.cidadao.php";
   ...
   if($id_documento == 10){ // É O CASO DE POSTO COMBUSTÍVEL
    ...
    ///$status = 2;  já determina que esse pedido vai direto para a homologação do delegado, já que é cadastrado internamente.
    $status = 0; // delegado determinou que deve aparecer para análise policial, assim como os outros.
    ...
    $taxa_valor = $_SESSION[valor_total_resumo]; // ESSE VALOR É SUBSTIUIDO PELO VALOR SOMADO DA TAXA E DA VISTORIA. O CALCULO ANTERIOR ESTAVA IGNORANDO A VISTORIA

    $query = "insert into tb_cidadao_pedidos (data_pedido, tipo_pedido, status, ultima_movimentacao, nome_estabelecimento, razao_social, id_ramo_atividade, id_atividade_ref, id_area_vistoria,cnpj, endereco_rua, endereco_numero, endereco_bairro, endereco_cep, id_municipio, telefone_fixo, telefone_celular, email, nome_proprietario, obs_cidadao, historico, senha, hash, valor_taxa, cidadao_nome, cidadao_cpf, requerente_nome, requerente_data_nasc, requerente_cpf, requerente_endereco, requerente_telefone, requerente_email, pedido_dispensa, pagina_resumo) values ($data_pedido, $id_documento, $status, $ultima_movimentacao, '$nome_estabelecimento', '$razao_social', '$id_ramo_atividade', $id_atividade_ref, $id_area_vistoria, '$cnpj', '$endereco_rua', '$endereco_numero', '$endereco_bairro', '$endereco_cep', $id_municipio, '$telefone_fixo', '$telefone_celular', '$email', '$nome_proprietario', '$obs_cidadao','$historico', '$senha_cidadao', '$hash', '$taxa_valor', '$cidadao_nome', '$cidadao_cpf', '$requerente_nome', '$requerente_data_nasc', '$requerente_cpf', '$requerente_endereco', '$requerente_telefone', '$requerente_email', '$pedido_dispensa', '$pagina_resumo')";
    //echo $query;
    $result = mysqli_query($link, $query);
    if(!$result)
    {   
      ...
      //  echo $query2;
        ...
        for($i2=0;$i2<$num2;$i2++) 
               ...
               //     $campo_valor_insere  = $f->limpa_variavel($_POST[$campo_nome], 250, $purifier);
                ...
                include "../cidadao/gera.resumo.empresa.bd.php";
       ...
       // echo " ==>".$id_documento_tipo;
        ...
        $extensao = $d2[1];
            ...
            if($extensao == 'pdf'){
                ...
                echo "<p><b>Não foi possível carregar o arquivo!</b></p>".$_FILES["arquivo"]["tmp_name"].$documentos_nome[$id_documento_tipo];
                }else{
